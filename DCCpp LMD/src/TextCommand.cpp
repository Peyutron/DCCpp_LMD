...
///
	...
	case 'F':     
		...
		*/
		...
		int v;
	  		...
	  		int mem = (int) &v - (__brkval == 0 ? (int) &__heap_start : (int) __brkval);
	...
	String info_vers	= "<iDCCpp LIBRARY BASE STATION FOR ARDUINO V-" + (String)DCCPP_LIBRARY_VERSION + ">";
	String info_serial	= "<N Wifi: " + ser1 + " Bluetooth: " + ser2 +  " Aux: " + ser3  + ">";
	// CommManager::printf(vers);
	#ifdef USE_SERIALWIFI
		WIFI.println(info_vers);
		WIFI.print(info_serial);
	#endif
	#ifdef USE_SERIALBLUETOOTH
		BLUETOOTH.println(info_vers);
		BLUETOOTH.print(info_serial);
	#endif
	#ifdef USE_SERIALAUX
		SERIALAUX.print(info_vers);
